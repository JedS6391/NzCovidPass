# Azure function demo

Illustrates usage of the verifier as part of an Azure Function HTTP trigger. The function can be invoked to verify the contents of a pass and return the result.

## Usage

```http
POST /api/VerifyPass HTTP/1.1
 
{
    "passPayload": "NZCP:"
}
 
HTTP/1.1 200 OK
 
{
    "isValid": true,
    "givenName": "Jack",
    "familyName": "Sparrow",
    "dateOfBirth": "1960-04-16T00:00:00+12:00",
    "expiresOn": "2031-11-02T20:05:30+00:00"
}
```
